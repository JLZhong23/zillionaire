...
void GameStart() {
    ...
    while (1) {
        ...
        if (!game_state->current_player->sleep_time) {
            ...
            else if (STR_EQU(COMMAND_STORE_TEST,com_buf)){
                StoreTheGameState();
...
bool StoreTheGameState(){
    FILE * fp=NULL;
    fp=fopen("./store_file.txt","w+");
    char * string_to_store=NULL;
    ...
    sprintf(str, "%d\n", game_state->player_num);    //printf("Original number; %f\n" , num);
    printf ("player num to save %s\n",str);
    fputs(str,fp);

    //save all player state
    sprintf(str,"%d\n",game_state->current_player->player_id);
    printf ("current player id to save %s\n",str);
    fputs(str,fp);

    PLAYER * saving_player=game_state->player;
    for(int i=0;i<game_state->player_num;i++){
        //save id 
        sprintf(str,"%d\n",saving_player->player_id);
        printf ("%d player id to save %s\n",saving_player->player_id,str);
        fputs(str,fp);
        //save player name
        fputs(saving_player->player_name,fp);fputs("\n",fp);
        //save player position 
        sprintf(str,"%d\n",saving_player->cur_pos);
        printf ("%d player position to save %s\n",saving_player->player_id,str);
        fputs(str,fp);
        //save the tool num
        sprintf(str,"%d\n",saving_player->bomb_num);
        fputs(str,fp);
        sprintf(str,"%d\n",saving_player->block_num);
        fputs(str,fp);
        sprintf(str,"%d\n",saving_player->robot_num);
        fputs(str,fp);
        //save the skip rank
        sprintf(str,"%d\n",saving_player->sleep_time);
        fputs(str,fp);
        //save number of player don`t pay the rent
        sprintf(str,"%d\n",saving_player->no_rent_time);
        fputs(str,fp);
        //money 
        sprintf(str,"%d\n",saving_player->money);
        fputs(str,fp);
        ...
        saving_player=saving_player->next;
    }

    fputs(game_state->path,fp);fputs("\n",fp);
    fputs("This is saving file end\n", fp);
    fclose(fp);
